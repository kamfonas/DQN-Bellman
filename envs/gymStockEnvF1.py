...
LAST_PRICE = 1 # Last known price (yesterday's adj close )
POSITION = 2 # Position Stock quantity
STATE_BUFFER_LEN = 3
MAX_LONG = 100
MAX_SHORT = -100
SCALING_FACTOR = 10

class StockEnvF1(gym.Env):
    ...
    def __init__(self, df,day = 0):
        ...
        self.initPrice = df[0, 20]
        self.df_BuyHold = self.df.iloc[:,20]*INITIAL_ACCOUNT_BALANCE/self.initPrice
        ...
        self.data = list([self.df.iloc[self.day, 20]])
        # print(self.data)
        self.terminal = False             
        # initalize state
