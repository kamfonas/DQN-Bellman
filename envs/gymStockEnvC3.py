...
DATASOURCE_START  = 2      # data read-in from data source
DATASOURCE_END    = 37
DATASOURCE_LEN    = DATASOURCE_END - DATASOURCE_START
...
class StockEnvC3(gym.Env):
    ...
    def __init__(self, df,day = 0,runPath='episodes',runType='train'):
        ...
        self.initPrice = df[0,0]
        self.df_BuyHold = self.df.iloc[:,0]*INITIAL_ACCOUNT_BALANCE/self.initPrice

        # action_space allows 3 actions in one dimension (we only trade one stock)
        self.action_space = spaces.Discrete(2) # In, Out
        # Observation space  
    ...
    def step(self, actions):
        ...
        if self.terminal:  # If reached end of episode...
            ...
            if self.episode % 1 == 0: 
                title = "Episode {} vs Buy and Hold ({})".format(self.episode,self.runType)
                ...
                sellIx = np.array([i for i, x in enumerate(self.action_memory) if x==0 if i < L])
            ...
            return self.state[STATE_START:STATE_END], self.reward, self.terminal,{
                'sharpe':sharpe}
            ...
            if actions == 1:
                self._buy()
            self.day += 1
            self.data = list(self.df.iloc[self.day,0:DATASOURCE_LEN])

            #load next state
            self.state = [self.state[ACCT_BAL],self.state[POSITION]]+\
                     self.data + [self.state[INVESTED]] 
    ...
    def reset(self):
        ...
        self.data = list(self.df.iloc[self.day,0:DATASOURCE_LEN])
        self.state = [INITIAL_ACCOUNT_BALANCE,0]+\
                     self.data +\
                     [0]
        print('Initializing State with zero position & initial Investment in cash' )
        print('data read:',len(self.data), '"Invested" set to 0 by default')
        self.cost = 0
        self.reward = 0
        self.terminal = False 
        self.rewards_memory = []
        ...
        return self.state[STATE_START:STATE_END]
